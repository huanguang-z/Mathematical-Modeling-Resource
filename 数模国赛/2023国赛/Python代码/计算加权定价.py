...
for i in tqdm(range(len(date))):
    ...
    price['花叶类加权利润率']=(price['花叶类加权定价']-price['花叶类加权成本'])/price['花叶类加权定价']
    price['花菜类加权利润率']=(price['花菜类加权定价']-price['花菜类加权成本'])/price['花菜类加权定价']
    price['水生根茎类加权利润率']=(price['水生根茎类加权定价']-price['水生根茎类加权成本'])/price['水生根茎类加权定价']
    price['茄类加权利润率']=(price['茄类加权定价']-price['茄类加权成本'])/price['茄类加权定价']
    price['辣椒类加权利润率']=(price['辣椒类加权定价']-price['辣椒类加权成本'])/price['辣椒类加权定价']
    price['食用菌加权利润率']=(price['食用菌加权定价']-price['食用菌加权成本'])/price['食用菌加权定价']

    result=result.append(price,ignore_index=True)

result=pd.merge(result,shoujia,on="日期",how="left")
result.to_csv("加权定价.csv",index=False)
result.to_excel("加权定价.xlsx",index=False,sheet_name="Sheet1")
